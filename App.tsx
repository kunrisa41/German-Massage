    ...
    <div className="min-h-screen font-sans bg-[#FDF5EF] text-[#2C1810] selection:bg-[#D4AF37] overflow-x-hidden">
      ...
      <main>
        ...
        <section id="hero" className="relative h-[65vh] md:h-[75vh] flex items-center justify-center bg-[#FDF5EF] overflow-hidden">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-[#2D1810]/60 via-transparent to-white/10"></div>
          
          <div className="relative z-10 text-center px-4 max-w-4xl">
            <h1 className="font-serif mb-4 leading-tight animate-fadeIn text-white drop-shadow-[0_4px_12px_rgba(0,0,0,0.8)]">
               <span className="text-4xl md:text-7xl block tracking-tight font-bold">{t.hero.title}</span>
               <span className="text-lg md:text-2xl block mt-2 opacity-95 font-light italic tracking-wide uppercase">{t.hero.subtitle}</span>
            </h1>
            <p className="text-white text-xs md:text-base mb-8 font-semibold tracking-[0.25em] uppercase animate-fadeIn max-w-xl mx-auto drop-shadow-md">
              {t.hero.tagline}
            </p>
            <div className="flex justify-center animate-fadeIn">
              <button onClick={() => scrollToSection('treatments')} className="px-12 py-4 rounded-xl font-bold tracking-widest uppercase text-[10px] shadow-2xl transition-all hover:scale-105 hover:bg-[#4a0a0a] bg-[#720e0e] text-white border border-[#D4AF37]/40">{t.hero.cta}</button>
            </div>
          </div>
        </section>
        ...
        <section className="py-12 md:py-20 px-6 bg-[#FDF5EF]">
          <div className="max-w-3xl mx-auto text-center">
            ...
            <div className="space-y-6 text-[#2D1810]/80 text-sm md:text-base leading-relaxed font-light">
              <p className="italic font-medium text-[#4B2C20] text-lg">{t.philosophy.p1}</p>
        ...
        <section id="treatments" className="py-16 md:py-24 px-4 bg-[#3E2723]">
          <div className="max-w-4xl mx-auto">
            <div className="text-center mb-14">
              ...
              <h2 className="text-4xl md:text-6xl font-serif mb-4 text-white font-bold">{t.treatments.title}</h2>
              <div className="w-16 h-0.5 mx-auto bg-[#D4AF37]"></div>
            </div>

            <div className="grid gap-8">
              {services.map((service) => (
                <div key={service.id} className="rounded-3xl overflow-hidden flex flex-col md:flex-row shadow-2xl bg-white border border-[#D4AF37]/10">
                  <div className="md:w-[65%] p-8 md:p-10">
                    <span className="text-[9px] font-bold tracking-widest uppercase py-1.5 px-4 rounded-full bg-[#3E2723] text-[#D4AF37] mb-5 inline-block">{service.subtitle}</span>
                    <h3 className="text-3xl md:text-4xl font-serif mb-3 text-[#3E2723] leading-tight font-bold">{service.title}</h3>
                      ...
                      <div>
                        ...
                        <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3">
                          {service.highlights.map((h, i) => (
                            <li key={i} className="flex items-start text-xs md:text-sm text-[#2D1810]/90 font-medium leading-snug">
                              <span className="mr-2 text-[#D4AF37] mt-1 shrink-0">•</span>
                              {h}
                            </li>
                          ))}
                        </ul>
                        ...
                        <div>
                          ...
                          <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3">
                            {service.effects.map((e, i) => (
                              <li key={i} className="flex items-start text-xs md:text-sm text-[#3E2723] font-bold leading-snug">
                                <span className="mr-2 text-[#D4AF37] mt-1 shrink-0">✦</span>
                                {e}
                              </li>
                            ))}
                  ...
                  <div className="md:w-[35%] p-8 md:p-10 flex flex-col items-center justify-center text-center border-t md:border-t-0 md:border-l border-stone-100 bg-[#FDF5EF]/80">
                    ...
                    <div className="space-y-5 mb-8 w-full">
                      {service.options.map((opt, idx) => (
                        <div key={idx} className="flex flex-col items-center group">
                          <div className="text-5xl md:text-6xl font-serif text-[#3E2723] flex items-baseline leading-none mb-2 transition-transform group-hover:scale-105">
                             <span className="text-xl mr-1 text-[#D4AF37] font-sans font-bold">€</span>{opt.price}
                          </div>
                          <div className="text-[9px] font-bold text-white uppercase tracking-widest bg-[#3E2723] px-5 py-1.5 rounded-full shadow-md">{opt.duration}</div>
                        </div>
                      ))}
                    </div>
                    <a href={SHOP_INFO.phoneLink} className="w-full max-w-[180px] py-4 rounded-xl text-[10px] font-bold uppercase tracking-widest text-white shadow-xl transition-all hover:scale-105 bg-[#720e0e] hover:bg-[#4a0a0a]">
        ...
        <section className="py-20 px-4 bg-[#FDF5EF]">
          <div className="max-w-5xl mx-auto">
            <div className="text-center mb-12">
              <h2 className="text-3xl md:text-5xl font-serif text-[#3E2723] mb-4 font-bold">{t.anspruch.title}</h2>
              <div className="w-12 h-0.5 mx-auto bg-[#D4AF37]"></div>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {t.anspruch.items.map((item, i) => (
                <div key={i} className="p-6 rounded-3xl bg-white border border-[#D4AF37]/15 text-center hover:shadow-xl transition-all aspect-square flex flex-col justify-center items-center group">
                  ...
                  <p className="text-[#2D1810]/70 text-[11px] leading-relaxed font-light line-clamp-3">{item.text}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* Contact Section */}
        <section id="contact" className="py-20 md:py-24 px-4 bg-[#2D1810]">
          <div className="max-w-5xl mx-auto">
            <div className="grid md:grid-cols-2 gap-8">
              <div className="p-8 md:p-10 rounded-3xl bg-[#FDF5EF] flex flex-col justify-between shadow-2xl border-l-8 border-[#D4AF37]">
                <div>
                  <p className="text-[#D4AF37] text-[10px] font-black uppercase tracking-widest mb-5">{t.contact.label}</p>
                  <h3 className="text-3xl font-serif mb-8 text-[#3E2723] font-bold">{t.contact.title}</h3>
                  <div className="flex items-start space-x-4 mb-8">
                    <div className="p-3.5 rounded-xl bg-[#3E2723] text-[#D4AF37] shrink-0 shadow-lg">
                      <MapPin size={24} />
                    </div>
                ...
                <div 
                  className="relative h-40 rounded-2xl overflow-hidden group cursor-pointer border border-[#3E2723]/10 shadow-inner"
                  onClick={() => window.open(SHOP_INFO.googleMapsUrl, '_blank')}
                >
                   <img src="https://images.unsplash.com/photo-1526778548025-fa2f459cd5c1?q=80&w=1000&auto=format&fit=crop" alt="Location Map" className="absolute inset-0 w-full h-full object-cover grayscale brightness-75 group-hover:grayscale-0 transition-all duration-700" />
                   <div className="absolute inset-0 flex items-center justify-center">
                         <div className="bg-[#D4AF37]/95 p-3 rounded-full text-white shadow-2xl group-hover:scale-125 transition-transform">
                            <MapPin size={20} />
                         </div>
                   </div>
                </div>
              ...
              <div className="p-8 md:p-10 rounded-3xl flex flex-col justify-center items-center text-center bg-white/5 border border-white/10 shadow-2xl relative overflow-hidden">
                <div className="absolute top-0 right-0 w-32 h-32 bg-[#D4AF37]/5 rounded-full -mr-16 -mt-16 blur-3xl"></div>
                <p className="text-[#D4AF37] uppercase tracking-widest text-[9px] font-black mb-5">{t.contact.reservation}</p>
                <div className="bg-[#D4AF37]/10 p-6 rounded-full mb-6 text-[#D4AF37] shadow-inner">
                  <Phone size={44} />
                </div>
                <p className="text-white/40 text-[8px] mb-2 uppercase tracking-widest font-black">{t.contact.callUs}</p>
                <h2 className="text-4xl md:text-5xl font-serif text-white mb-10 tracking-tight font-bold">{SHOP_INFO.phone}</h2>
                <a href={SHOP_INFO.phoneLink} className="w-full max-w-[240px] py-4.5 bg-[#720e0e] text-white rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-[#4a0a0a] transition-all shadow-2xl hover:scale-105 active:scale-95">{t.contact.callCta}</a>
                <p className="mt-8 text-[8px] text-[#D4AF37]/50 uppercase tracking-widest font-black">{t.contact.note}</p>
              </div>
            </div>
          </div>
        </section>
      ...
      <footer className="py-12 px-4 bg-[#FDF5EF] border-t border-[#D4AF37]/20 text-center">
        <div className="max-w-6xl mx-auto">
          <div className="font-serif text-[#720e0e] text-2xl uppercase mb-3 font-bold tracking-tight">
             {t.hero.title}
          </div>
          <p className="text-[10px] tracking-[0.4em] opacity-60 text-[#3E2723] mb-6 uppercase font-bold">{t.footer.tagline}</p>
          <div className="w-8 h-px bg-[#D4AF37] mx-auto mb-6"></div>
